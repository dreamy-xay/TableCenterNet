...
Version: 
Autor: dreamy-xay
Date: 2024-10-22 20:25:24
LastEditors: dreamy-xay
LastEditTime: 2024-10-22 20:25:36
"""
import numpy as np
import cv2
import random
...
CONFIG = {"is_transform_simple": False}


def transform_preds(coords, center, scale, output_size, rot=0, shift=np.array([0, 0], dtype=np.float32)):
    if CONFIG["is_transform_simple"]:
        return transform_preds_simple(coords, center, scale, output_size, rot, shift)
...
def get_affine_transform(center, scale, rot, output_size, shift=np.array([0, 0], dtype=np.float32), inv=0):
    if CONFIG["is_transform_simple"]:
        return get_affine_transform_simple(center, scale, rot, output_size, shift, inv)
...
def transform_preds_simple(coords, center, scale, output_size, rot=0, shift=np.array([0, 0], dtype=np.float32)):
    target_coords = np.zeros(coords.shape)
    trans = get_affine_transform_simple(center, scale, rot, output_size, inv=1)
    for p in range(coords.shape[0]):
        target_coords[p, 0:2] = affine_transform(coords[p, 0:2], trans)
    return target_coords


def get_affine_transform_simple(center, scale, rot, output_size, shift=np.array([0, 0], dtype=np.float32), inv=0):
...
def color_aug(image, data_rng, eig_val, eig_vec):
