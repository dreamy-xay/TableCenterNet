...
def crate_json_object_element(cell, page_bbox, output_image_max_dim=1000):
    page_width = page_bbox[2] - page_bbox[0]
    page_height = page_bbox[3] - page_bbox[1]
    ...
    for idx, pdf_filepath in enumerate(pdf_filepaths):
        ...
        for table_dict in document_tables:
            ...
            for cell in table_dict['cells']:
                ...
                table_cells.append({'bbox': cell['pdf_bbox'], 'start_col': min(cell['column_nums']), 'end_col': max(cell['column_nums']), 'start_row': min(cell['row_nums']), 'end_row': max(cell['row_nums'])})
            ...
            for cell in table_cells:
                cell_json = crate_json_object_element(cell, [0, 0, table_img.size[0], table_img.size[1]],
                                                        output_image_max_dim=max(table_img.size))
            ...
            if do_save:
                ...
                json_filepath = os.path.join(output_structure_directory, split, table_image_filename.replace(".jpg", ".json"))
